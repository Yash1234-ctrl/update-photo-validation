...
class SmartFarmAssistant:
    def __init__(self):
        ...
        self.crop_requirements = {
            'Rice': {'water': 'High', 'soil_ph': '6.0-7.5', 'temperature': '20-35°C', 'season': 'Kharif'},
            'Wheat': {'water': 'Moderate', 'soil_ph': '6.0-7.5', 'temperature': '15-25°C', 'season': 'Rabi'},
            'Cotton': {'water': 'Moderate', 'soil_ph': '5.8-8.0', 'temperature': '21-30°C', 'season': 'Kharif'},
            'Sugarcane': {'water': 'High', 'soil_ph': '6.0-7.5', 'temperature': '21-27°C', 'season': 'Year-round'},
            'Tomato': {'water': 'Moderate', 'soil_ph': '6.0-6.8', 'temperature': '20-25°C', 'season': 'Winter'},
            'Potato': {'water': 'Moderate', 'soil_ph': '5.2-6.4', 'temperature': '15-20°C', 'season': 'Winter'},
        }
        ...
        try:
            if os.path.exists('best_model.h5'):
                ...
                if os.path.exists('class_names.txt'):
                    ...
                    self.class_names = ['Healthy', 'Early Blight', 'Late Blight', 'Bacterial Spot']
                
                st.success("✅ AI Disease Detection Model Loaded Successfully!")
            else:
                self.disease_model = None
                st.warning("⚠️ AI model not found. Disease detection will use sample data.")
        ...
        try:
            ...
            if self.disease_model is not None:
                try:
                    ...
                    st.warning(f"Model prediction failed: {model_error}. Using simulation.")
                ...
                diseases = ['Healthy', 'Early Blight', 'Late Blight', 'Bacterial Spot']
                disease = np.random.choice(diseases, p=[0.4, 0.25, 0.20, 0.15])  # Weighted probabilities
            ...
            recommendations = self.generate_crop_recommendations(disease, crop_type, health_score)
    ...
    def generate_crop_recommendations(self, disease, crop_type, health_score):
        ...
        recommendations.extend(disease_actions.get(disease.lower(), ["Consult agricultural expert for treatment"]))
    ...
    def get_weather_data(self, district, days=7):
        ...
        try:
            ...
            temperatures = np.clip(temp_base + temp_var * np.random.random(days), 5, 50).round(1)
            humidity = np.clip(humidity_base + humidity_var * np.random.random(days), 20, 100).round(1)
            rainfall = np.clip(np.random.exponential(rain_intensity, days), 0, 200).round(1)
    ...
    def create_weather_charts(self, weather_data):
        ...
        try:
            ...
            humidity_rain_fig.add_trace(go.Bar(
                x=weather_data['dates'],
            ...
            humidity_rain_fig.add_trace(go.Scatter(
                x=weather_data['dates'],
            ...
            temp_fig = go.Figure()
            temp_fig.add_trace(go.Scatter(x=[0, 1], y=[25, 25], mode='lines', name='Temperature'))
            temp_fig.update_layout(title='Temperature Chart (Error)', height=350)
            
            humidity_fig = go.Figure()
            humidity_fig.add_trace(go.Scatter(x=[0, 1], y=[60, 60], mode='lines', name='Humidity'))
    ...
    def analyze_soil_health(self, ph, nitrogen, phosphorus, potassium):
        ...
        if ph < 6.0:
            ...
            recommendations.append("Soil is alkaline - apply organic matter to reduce pH")
...
def main():
    ...
    with st.sidebar:
        ...
        crop_category = st.selectbox(
            "🌱 Crop Category",
            list(assistant.crop_types.keys())
        )
        
        selected_crop = st.selectbox(
            "🌾 Select Crop",
            assistant.crop_types[crop_category]
        )
    ...
    with tab1:
        ...
        with col1:
            ...
            """.format(selected_crop), unsafe_allow_html=True)
        ...
        with col2:
            ...
            """.format(farm_size), unsafe_allow_html=True)
            ...
            if uploaded_file:
                if st.button("🔍 Analyze Crop Health", type="primary", use_container_width=True):
                    with st.spinner("Analyzing crop health..."):
                        result = assistant.analyze_crop_health(uploaded_file, selected_crop)
    ...
    with tab3:
        ...
        with col2:
            st.plotly_chart(humidity_rain_fig, use_container_width=True)
        ...
        recommendations = []
        if avg_temp > 35:
            recommendations.append("🌡️ High temperatures - Increase irrigation frequency and provide shade")
        elif avg_temp < 15:
            recommendations.append("🌡️ Low temperatures - Use mulching and protect from frost")
        
        if total_rainfall > 100:
            recommendations.append("🌧️ Heavy rainfall - Ensure proper drainage to prevent waterlogging")
        elif total_rainfall < 10:
            recommendations.append("🌧️ Low rainfall - Plan for supplemental irrigation")
        
        if avg_humidity > 85:
            recommendations.append("💧 High humidity - Monitor for fungal diseases and improve ventilation")
        elif avg_humidity < 40:
            recommendations.append("💧 Low humidity - Increase irrigation and consider mulching")
        
        if not recommendations:
            recommendations.append("✅ Weather conditions are favorable for crop growth")
    ...
    with tab4:
        ...
        with col1:
            ...
            nitrogen = st.number_input("Nitrogen (N) - kg/ha", 0, 200, 45, 5, key="nitrogen_input")
        
        with col2:
            phosphorus = st.number_input("Phosphorus (P) - kg/ha", 0, 100, 30, 5, key="phosphorus_input")
            potassium = st.number_input("Potassium (K) - kg/ha", 0, 200, 40, 5, key="potassium_input")
        ...
        if st.button("🔍 Analyze Soil Health", type="primary"):
            soil_analysis = assistant.analyze_soil_health(soil_ph, nitrogen, phosphorus, potassium)
    ...
    with tab5:
        ...
        if st.button("💾 Save Current Analysis", type="primary"):
            if 'analysis_result' in st.session_state and 'soil_analysis' in st.session_state:
                ...
                record_data = (
                    selected_district, selected_crop, farm_size,
                    crop_result['health_score'], crop_result['disease'],
                    soil_ph_value, nitrogen_value, phosphorus_value, potassium_value,
        ...
        try:
            ...
            if not df.empty:
                ...
                st.info("📝 No records found. Start analyzing your crops to build history!")
