...
class MongoCropDB:
    ...
    def create_indexes(self):
        ...
        try:
            ...
            self.pest_predictions.create_index([('district', 1), ('prediction_date', -1)])
    ...
    def save_crop_analysis(self, analysis_data):
        ...
        try:
            ...
            analysis_doc = {
                'timestamp': datetime.now() if isinstance(analysis_data['timestamp'], str) else analysis_data['timestamp'],
            ...
            if result.inserted_id:
                print(f"[OK] Crop analysis saved successfully with ID: {result.inserted_id}")
                return result
            else:
    ...
    def get_farmer_history(self, farmer_id):
        ...
        try:
            return {
                'crop_analysis': list(self.crop_analysis.find(
                    {'farmer_id': farmer_id},
                    {'_id': 0}
                ).sort('timestamp', -1).limit(10)),
                'soil_analysis': list(self.soil_analysis.find(
                    {'farmer_id': farmer_id},
                    {'_id': 0}
    ...
    def get_weather_history(self, district, days=30):
        ...
        try:
            ...
            return list(self.weather_data.find(
                {
                    'district': district,
    ...
    def get_pest_alerts(self, district):
        ...
        if not self.connected:
            return []
        try:
            return list(self.pest_predictions.find(
                {
                    'district': district,
                    'risk_level': {'$in': ['High', 'Critical']}
                },
                {'_id': 0}
            ).sort('prediction_date', -1).limit(5))
        except Exception as e:
